...
`graft build` за замовчуванням додає ребра викликів компіляторної точності, якщо встановлено `rust-analyzer`, `clangd`, `gopls`, `pyright` або `typescript-language-server` (вимкнути: `--no-lsp` або `GRAFT_NO_LSP=1`). Файл із непідтримуваним розширенням пропускається й потрапляє у звіт, а не обробляється навмання.

- **Загальне витягування** — Rust, C і C++ використовують tags-запити для символів і викликів.
- **Ребра компіляторної точності (за замовчуванням)** — `graft build` додає точні
  ...
  Фонова синхронізація з хуків мовний сервер не запускає; `--no-lsp` вимикає збагачення.
...
graft build --no-lsp                 # не запускати мовний сервер (за замовчуванням він використовується, якщо є в PATH)
